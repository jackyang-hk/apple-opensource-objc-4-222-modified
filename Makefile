...
info:
	$(ECHO) "info: $(shell uname) $(ARCHS) $(CC) $(CFLAGS_PROFILE)   "$<" -c -o "$@" $(OBJROOT)"


.SUFFIXES:
.PHONY: default all build optimized debug profile installsrc installhdrs install clean prebuild build-optimized build-debug build-profile prebuild-optimized prevuild-debug prebuild-profile compile-optimized compile-debug compile-profile link-optimized link-debug link-profile postbuild info
...
#CC = /usr/bin/cc
CC = /Applications/Xcode_4.5.2.app/Contents/Developer/Toolchains/XcodeDefault.xctoolchain/usr/bin/clang
else
CC = /usr/bin/gcc
endif
